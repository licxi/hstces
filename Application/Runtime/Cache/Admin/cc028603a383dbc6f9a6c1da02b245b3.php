...
<div class="my_table">
  <h2>管理员列表 <a class="btn btn-info navbar-right" data-toggle="modal" data-target="#add_admin">添加管理员</a></h2><hr>
      <?php if($admins_list): ?><div class="table-responsive">
        <table class="table table-hover table-bordered table-striped center">
          ...
          <tbody>
            ...
            <?php if(is_array($admins_list)): foreach($admins_list as $key=>$vo): ?><tr id="tr<?php echo ($vo["id"]); ?>">
              <td ><?php echo ($vo["id"]); ?></td>
              <td ><?php echo ($vo["admin_name"]); ?></a></td>
              <td ><?php echo ($vo["nickname"]); ?></td>
              <td ><button class="btn btn-default" onClick="del(<?php echo ($vo["id"]); ?>)">删除</button> </td>
...
<script>
  function del(id) {
    if(confirm("确认删除该管理员")){
      var url = "<?php echo U('admin/admin/del');?>";
