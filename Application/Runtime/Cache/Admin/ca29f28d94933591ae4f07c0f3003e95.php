...
<!-- 头部结束-->
<div class="my_table">
  <h2>文件列表<span style="padding-left: 40%"><?php echo ($title); ?></span></h2><hr>
  <div class="search">
        <form id="searchForm" name="searchform" class="well form-search"
          ...
          <div class="row">
            ...
            <div class="col-sm-6 col-lg-6">
              ...
              <a class="btn btn-info navbar-right" href="<?php echo U('admin/file/fileUpload','','');?>">上传文件</a>
            </div>
          </div>
        </form>
    </div>
      <?php if($files_list): ?><div class="dataBox">
      <div class="table-responsive">
        <table class="table table-hover table-bordered table-striped center">
          ...
          <tbody>
            ...
            <?php if(is_array($files_list)): foreach($files_list as $key=>$vo): ?><tr id="tr<?php echo ($vo["file_id"]); ?>">
              <td ><?php echo ($vo["file_id"]); ?></td>
              <td ><?php echo ($vo["file_name"]); ?></td>
              <td ><?php echo ($vo["file_describe"]); ?></td>
              <td ><?php echo ($vo["file_ext"]); ?></td>
              <td ><?php echo (size($vo["file_size"])); ?></td>
              <td >
                <a class="btn btn-default" href="<?php echo U("admin/file/modify?file_id=$vo[file_id]",'','');?>">编辑文件描述</a>
                <a class="btn btn-default" href="<?php echo U("/admin/file/download?file_id=$vo[file_id]",'','');?>">下载</a>
                <button id="del" class="btn btn-danger" onclick="del_file(<?php echo ($vo["file_id"]); ?>,'<?php echo ($vo["file_name"]); ?>')">删除</button>
              </td>
...
<script type="text/javascript">
	function del_file(file_id,file_name){
		if(confirm('是否删除   '+file_name+"  删除后不可恢复!")){
			 var url = "<?php echo U('admin/file/del');?>";
