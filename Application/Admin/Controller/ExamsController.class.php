...
class ExamsController extends AuthController {
	...
	public function save() {
		...
		if ($exams_id !== "") {
			...
			$result = $exams->where($where)->save($data);
			if ($result) {
				$_SESSION ["exam_id"] = $exams_id;
				$this->success("修改成功！",U("admin/exams/getexams?".time(),"",""));
			} else {	
				$this->error ( "修改失败，请重试！" );
			}
		} else {
			...
			if ($result > 0) {
				...
				/*$this->assign ( "exam_id", $result);
				$this->display ( ":Questions/Questions_import" ); */
				$_SESSION ["exam_id"] = $result;
				$this->redirect("admin/questions/import/exam_id/$result");
	...
	public function getExams() {
		...
		if ($total > 0) {
			$exams_list = $exams->where ( $filter )->limit ( $Page->firstRow . ',' . $Page->listRows )->select ();
		}
		// 对数据进行一定的转换
		foreach ( $exams_list as $key => $exam ) {
			$exam_id = $exam ["exam_id"];
			$total = $scores->where ( array (
					"exam_id" => $exam_id 
			) )->count ();
			$total_number = array (
					"total_number" => $total 
	...
	public function changeExamStatus() {
		...
		if ($exam_id == "") {
			...
			if ($status == 1) {
				$result = $exams->where ( $exam_id )->setField ( array (
						"status" => 0 
				) );
				if ($result) {
					$data = array (
